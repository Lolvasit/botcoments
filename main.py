...
async def join_channel(account: str, link: str, client=None):
    try:
        if client is None:
    ...
    if link.startswith("https://t.me/+"):
        ...
        try:
            ...
            print(f"channel {link}, account {account}")
            traceback.print_exc()
            if account != "main.session":
                await client.disconnect()
    else:
...
async def leave_channel(account: str, chat_id: int, client=None):
    ...
    try:
        ...
        print(f"channel {chat_id}, account {account}")
        traceback.print_exc()
...
async def proxys_get(msg: Message):
    ...
    for channel in msg.get_args().split("\n"):
        for account in get_accounts_dir():
            if account == "main.session":
                chat_id = await leave_channel(account, channel, client=main_client)
            else:
                chat_id = await leave_channel(account, channel)
        channels.pop(str(chat_id))
    with open("channels.txt", "w") as f:
        json.dump(channels, f)
    await msg.answer("Успішно!")
    channels_list = load_json("channels.txt")
...
async def my_event_handler(event):
    global count_posts
    channels_list = load_json("channels.txt")
    if str(event.message.chat.id) not in channels_list.keys():
        return
